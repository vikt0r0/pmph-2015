...
flatQuicksort ne sizes arr =
    -- if all elements in sizes are less than 2, return arr
    if reduce (&&) True $ map (\s->(s<2)) sizes then arr
            -- si contains indices of subarrays in arr, e.g.
            -- for sizes=[3,0,0,2,0,5,0,0,0,0,1], then
            -- si=[3,3,3,5,5,10,10,10,10,10,11]
    else let si = scanInc (+) 0 sizes
             -- we now compute indices of, that is:
             -- (zip3 (0:si) si sizes)=[(0,3,3),(3,3,0),(3,3,0),(3,5,2),(5,5,0),(5,10,5),(10,10,0),(10,10,0),(10,10,0),(10,11,0)]
             r_inds= map (\(l,u,s)-> if s<1 then ne
                                -- pick a random index between lower and upper and extract the pivot
                                else let i :: Int
...
segmSpecialFilter cond sizes arr = let
    ...
    n     = length arr
    offs  = scanInc (+) 0 sizes
    offs' = zipWith (-) offs (segmScanInc (+) 0 sizes sizes)
    cs    = map cond arr
    tfs   = map (\f -> if f then 1 else 0) cs
    isT   = segmScanInc (+) 0 sizes tfs
    is    = map (\o -> isT !! (o-1)) offs
    ffs   = map (\f -> if f then 0 else 1) cs
    isF   = zipWith (+) is $ segmScanInc (+) 0 sizes ffs
    inds  = zipWith (+) offs' $ map (\(c,iT,iF) ->
              if c then iT-1 else iF-1)
              (zip3 cs isT isF)
    -- Compute flags
    tflags     = zipWith (\s i -> if s > 0 then i else 0) sizes is
    fflags = zipWith (-) sizes tflags
    fflag_idxs = zipWith3 (\tf ff o ->
                   if   tf > 0
                   then tf+o+1
                   else (if   ff > 0
                         then o+1
    ...
    flags  = tail $ write fflag_idxs fflags (0:tflags)
...
nestSparseMatVctMult mat x =
    map (\row -> let (inds,vals) = unzip row
                     v1 = map (\i -> x !! i) inds
                     ps = map (\(a,b) -> a*b) $ zip v1 vals -- zipWith
                 in  reduce (+) 0.0 ps
        ) mat
...
flatSparseMatVctMult flags mat b =
    let
      ...
      (ind,vals) = unzip $ map (\(f,o,s) -> if f > 0 then (o,s) else (0,0.0)) (zip3 flagl offs sums)
