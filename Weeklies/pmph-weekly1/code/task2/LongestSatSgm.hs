        ...
        lssop (lssx, lisx, lcsx, tlx, firstx, lastx, okx)
          ...
          (newlss, newlis, newlcs, tlx+tly, firstx, lasty, newok)
                where
                  ...
                  newlss  = if connect then (lcsx + lisy) `max` lssx `max` lssy else lssy `max` lssx
                  newlis  = if connect && okx then tlx + lisy else lisx
                  newlcs  = if connect && oky then tly + lcsx else lcsy
...
main = do args <- getArgs
          let inp :: [Int]
              inp = if null args then [1,2,3,3,2,5,6] else read (head args)
